...
export class SymptomRepository implements ISymptomRepository {
  async getAll(): Promise<SymptomEntity[]> {
    try {
      const response = await apiClient.get<SymptomEntity[]>('/symptoms');
  ...
  async getById(id: string): Promise<SymptomEntity> {
    try {
      const response = await apiClient.get<SymptomEntity>(`/symptoms/${id}`);
      return response.data;
    } catch (error: any) {
  ...
  async getByCategory(category: string): Promise<SymptomEntity[]> {
    try {
      const response = await apiClient.get<SymptomEntity[]>(`/symptoms/category/${category}`);
      return response.data;
    } catch (error: any) {
  ...
  async getCategories(): Promise<SymptomCategoryEntity[]> {
    try {
      const response = await apiClient.get<SymptomCategoryEntity[]>('/symptoms/categories');
      return response.data;
    } catch (error: any) {
  ...
  async saveUserSymptoms(data: UserSymptomRecordEntity): Promise<any> {
    try {
      const response = await apiClient.post('/user-symptoms', data);
      return response.data;
    } catch (error: any) {
  ...
  async getLatestUserSymptoms(userId: string): Promise<UserSymptomRecordEntity | null> {
    try {
      const response = await apiClient.get<UserSymptomRecordEntity>(`/user-symptoms/latest/${userId}`);
      return response.data;
    } catch (error: any) {
